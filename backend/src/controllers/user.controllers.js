...
exports.registroUsuario = async (req, res) => {
  ...
  if (correo.endsWith("@alumnos.ulagos.cl")) {
    ...
    return res
      .status(400)
      .json({
        error: "El correo electrónico debe ser de la Universidad de Los Lagos",
      });
  }
  try {
    ...
    if (!contraseñaRegex.test(contraseña)) {
      return res
        ...
        .json({
          error:
            "Tu contraseña debe tener 10 caracteres, una mayúscula y un símbolo..",
        });
  ...
  try {
    ...
    if (
      ...
      if (result.rows[0].contraseña !== contraseña) {
        return res
          .status(400)
          .json({ error: "Combinacion de correo y contraseña es incorrecta." });
      }
...
exports.eliminarVehiculo = async (req, res) => {
  ...
  try {
    ...
    await pool.query("DELETE FROM poseer_vehiculo WHERE patente = $1", [
      patente,
    ]);
...
exports.selectReservas = async (req, res) => {
  ...
  try {
    ...
    console.log(
      "resultado de consulta en selectReservasReportes:",
      result.rows
